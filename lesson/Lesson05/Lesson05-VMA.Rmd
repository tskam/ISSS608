...
There are 13 variables in this data set.  11 of them are in continuous data type, one in ordinal scale and one in norminal scale
.center[
![:scale 70%](img/image5-1.jpg)]
...
.pull-left[
.large[
+ When a relatively large number of obversation points is used, the scatterplot matrix failed to reveal the pair relationship effectively.
]]
...
---
## R package for creating Interactive Heatmap
...
packages = c('seriation', 'dendextend', 'heatmaply', 'readr')
...
Parallel Coordinates plot or Parallel plot allows to compare the feature of several individual observations on a set of numeric variables. Each vertical bar represents a variable and usually has its own scale. (The units can even be different). Values are then plotted as series of lines connected across each axis.

.center[
![:scale 80%](img/image8-8.jpg)]

---
## parallelPlot Package

.pull-left[
- An R package specially designed to plot a parallel coordinates plot by using 'htmlwidgets' package and [d3.js](https://d3js.org/).

- Before we get started, you should review the [Introduction to ‘parallelPlot’](https://cran.r-project.org/web/packages/parallelPlot/vignettes/introduction-to-parallelplot.html) to have an overall understanding of the features and functions of parallelPlot package.  You are also required to have the [user manual](https://cran.r-project.org/web/packages/parallelPlot/parallelPlot.pdf)of the package handy with you for reference purposes. 
]

.pull-right[

The code below is used to plot a basic parallel coordinates by using World Hapiness 2018 data set.
...
The code chunk below plot histograms on the axis of the parallel coordinates plot. 
