...
```{r}
cc_edges <- read_csv("data/cc_edges.csv")
cc_nodes <- read_csv("data/cc_nodes.csv")
```


```{r}
cc_edges$timestamp <- date_time_parse(cc_edges$timestamp,
                zone = "",
                format = "%m/%d/%Y %H:%M")
cc_edges$from <- as.character(cc_edges$from)
```
...
library(pagedown)
pagedown::chrome_print("In-class_Ex09-NetworkVis.html")
```
