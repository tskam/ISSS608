...
packages = c('corrplot', 'qtlcharts', 'ggpubr', 'plotly', 'tidyverse', 'ggstatsplot')
...
## 5.3 Including with correlation coefficients
...
# 6.0 Visualising Correlation Matrix: ggcormat()
...
#  7.0 Visualising Correlation Matrix using **corrplot** Package
...
### 7.6.1 Reordering a correlation matrix using **hclust**
...
# 8.0 Creating Interactive Correlation Matrix with Plotly R

In this section, you will learn how to draw an interactive scatter plot matrix by using an R package called [**qtlcharts**](https://academic.oup.com/genetics/article/199/2/359/5935788?login=false).  It is specially designed to provide interactive data visualizations for quantitative trait locus (QTL) mapping experiments. qtlcharts is a companion to **qtl** (Broman et al. 2003), providing interactive versions of qtl’s basic static graphics and additional interactive visualizations for high-dimensional genotype and phenotype data.

## 8.1 Data import and preparation

For the purpose of this hands-on exercise, World Happiness Data for 2018 will be used.  It is called WHData-2018.csv and can be found in the data sub-folder of Hands-on_Ex05.

First, we will important the data into R and parse it into an R object called *wh*. 

```{r}
wh <- read_csv("data/WHData-2018.csv")
```

wh is a tibble data table but qtlcharts requires the input data in matrix form.  The code chunk below is used to transform the wh data table into a matrix called *wh_m*.

```{r}
row.names(wh) <- wh$Country
wh1 <- dplyr::select(wh, c(3, 7:12))
wh_matrix <- data.matrix(wh)
wh_m <- wh_matrix[, !colnames(wh_matrix) %in% c("Country", "Region")]
```

## 8.1 Building the interactive correlation matrix

To prepare the interactive correlation matrix, iplotCorr() of qtlcharts will be used as shown in the code chunk below. 

```{r}
iplotCorr(wh_m, reorder=TRUE)
```
